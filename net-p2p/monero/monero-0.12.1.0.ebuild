...
# Keep this in sync with external/{miniupnp,unbound}
...
src_prepare() {
	rmdir external/{miniupnp,unbound} || die

	# Move dependencies
	mv "${WORKDIR}/${MINIUPNP_P}" \
		external/miniupnp || die
	if ! use system-unbound; then
		mv "${WORKDIR}/${UNBOUND_P}" \
			external/unbound || die
	fi
